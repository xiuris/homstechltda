...
PowerShell -command "&{(Get-Content -Path '%dirPHP%\php.ini') -replace ';extension=gd', 'extension=gd' -replace ';extension=zip', 'extension=zip'} | Set-Content -Path '%dirPHP%\php.ini'"
ECHO.
...
FOR /F %%A IN (' findstr /b /c:"EMAIL_PROTOCOL" "%dirMaposConfig%" ') DO SET "E_PROTO=%%A"
FOR /F %%A IN (' findstr /b /c:"EMAIL_SMTP_HOST" "%dirMaposConfig%" ') DO SET "E_HOST=%%A"
FOR /F %%A IN (' findstr /b /c:"EMAIL_SMTP_CRYPTO" "%dirMaposConfig%" ') DO SET "E_CRYPT=%%A"
FOR /F %%A IN (' findstr /b /c:"EMAIL_SMTP_PORT" "%dirMaposConfig%" ') DO SET "E_PORT=%%A"
FOR /F %%A IN (' findstr /b /c:"EMAIL_SMTP_USER" "%dirMaposConfig%" ') DO SET "E_USER=%%A"
FOR /F %%A IN (' findstr /b /c:"EMAIL_SMTP_PASS" "%dirMaposConfig%" ') DO SET "E_PASS=%%A"
PowerShell -command "&{(Get-Content -Path '%dirMaposConfig%') -replace '%%E_PROTO%%', 'EMAIL_PROTOCOL=%protocolo%' -replace '%%E_HOST%%', 'EMAIL_SMTP_HOST=%hostsmtp%' -replace '%%E_CRYPT%%', 'EMAIL_SMTP_CRYPTO=%criptografia%' -replace '%%E_PORT%%', 'EMAIL_SMTP_PORT=%porta%' -replace '%%E_USER%%', 'EMAIL_SMTP_USER=%email%' -replace '%%E_PASS%%', 'EMAIL_SMTP_PASS=%senha%'} | Set-Content -Path '%dirMaposConfig%'"
GOTO etapa0
