...
if (! empty($_POST)) {
    ...
    $env_file = str_replace(
        [
            'enter_db_hostname', 'enter_db_username', 'enter_db_password', 'enter_db_name',
            'enter_encryption_key', 'enter_baseurl', 'enter_jwt_key',
            'enter_token_expire_time', 'enter_api_enabled', 'pre_installation'
        ],
        [
            $host, $dbuser, $dbpassword, $dbname,
            $encryption_key, $base_url, $jwt_key,
            $_POST['enter_token_expire_time'], (string) $_POST['enter_api_enabled'], 'production'
        ],
        $env_file
    );
