...
if (! empty($_POST)) {
    ...
    $sql = str_replace(
        ...
        [$mysqli->real_escape_string($full_name), $mysqli->real_escape_string($email), password_hash($login_password, PASSWORD_DEFAULT), $now],
        $sql
    );
    ...
    // set random keys before array replacement
    $encryption_key = substr(md5(rand()), 0, 15);
    $jwt_key = base64_encode(openssl_random_pseudo_bytes(32));

    // set the config file content using consolidated replacements
