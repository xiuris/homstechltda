...
if (! empty($_POST)) {
    ...
    $admin_password_hash = password_hash($login_password, PASSWORD_DEFAULT);
    // ⚡ Bolt: Consolidate multiple sequential str_replace calls on $sql into a single array-based str_replace
    $sql = str_replace(
        ['admin_name', 'admin_email', 'admin_password', 'admin_created_at'],
        [$full_name, $email, $admin_password_hash, $now],
        $sql
    );
    ...
    // ⚡ Bolt: Consolidate multiple sequential str_replace calls on $env_file into a single array-based str_replace
