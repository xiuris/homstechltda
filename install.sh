...
# <=== Inicio Configurações Personalizadas ===>
    ...
    # <=== Inicio Configuração de dados de E-mail ===>
        ...
        if [ "$resposta" = "N" ] || [ "$resposta" = "n" ]; then
            ...
            # ⚡ Bolt: Consolidate sed commands into a single invocation to reduce process spawns and file I/O operations
            sed -i \
                -e "s/\EMAIL_PROTOCOL.*/\EMAIL_PROTOCOL='$protocolo'/" \
                ...
                -e "s/\EMAIL_SMTP_PASS.*/\EMAIL_SMTP_PASS='$senha'/" \
                $dirMaposConfig
            echo
